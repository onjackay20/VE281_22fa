...
public:
    ...
    private:
        ...
        const HashTable_copy *hashTable;
        ...
        explicit Iterator(HashTable_copy *hashTable) : hashTable(hashTable)
        {
        ...
        Iterator(HashTable_copy *hashTable, VectorIterator vectorIt, ListIterator listItBefore) : hashTable(hashTable), bucketIt(vectorIt), listItBefore(listItBefore)
        {
            endFlag = bucketIt == hashTable->buckets.end();
        }

    public:
        friend class HashTable_copy;
...
protected:                                                                 // DO NOT USE private HERE!
    ...
    void copy_from(const HashTable_copy &ht)
    {
        buckets.clear();
...
public:
    HashTable_copy() : buckets(DEFAULT_BUCKET_SIZE), tableSize(0), maxLoadFactor(DEFAULT_LOAD_FACTOR),
                  hash(Hash()), keyEqual(KeyEqual())
    {
        firstBucketIt = buckets.end();
    }

    explicit HashTable_copy(size_t bucketSize) : tableSize(0), maxLoadFactor(DEFAULT_LOAD_FACTOR),
                                            hash(Hash()), keyEqual(KeyEqual())
    {
    ...
    HashTable_copy(const HashTable_copy &that)
    {
        // TODO: implement this function
        if (&that != this)
        {
    ...
    HashTable_copy &operator=(const HashTable_copy &that)
    {
        // TODO: implement this function
        if (&that != this)
        {
    ...
    void rehash(size_t bucketSize)
    {
        ...
        // TODO: implement this function
        HashTable_copy big_table(bucketSize);
